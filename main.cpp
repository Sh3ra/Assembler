...
vector <regex> initializeRegexVector() {
    ...
    string declarationRes = "[a-z]\\w*\\s(resb|resw)\\s\\d{1,4}";
    string declarationByte = "[a-z]\\w*\\s(byte)\\s(x'([a-f0-9]{0,14})'|c'(\\D|\\S){0,15}')";
    string declarationWord = "[a-z]\\w*\\s(word)\\s-?\\d{1,4}";
    regex declaration ("(" + declarationWord + "|" + declarationByte  + "|" + declarationRes + ")");
    regexVector.push_back(declaration);
    regex access ("([a-z]\\w*\\s)?\\+?(st|ld)(x|a|b|s|t|ch)\\s(\\*|@?[a-z]\\w*(,[a-z]\\w*)?|#(\\d{1,4}|\\w*))");
    regexVector.push_back(access);
    regex jump ("([a-z]\\w*\\s)?(td|rd|wd|jeq|jlt|jle|jge|j|jgt|jsub|tixr)\\s(\\*|[a-z]\\w*)");
    regexVector.push_back(jump);
    regex rsub ("rsub");
    regexVector.push_back(rsub);
    regex commentLine ("\\.(\\W|\\S)*");
